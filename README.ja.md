...
- **Lean 4 で形式的に証明されています。** [`lean/`](lean/README.md) には193個の定理が収められており、いずれも Mathlib を用いて Lean のカーネルで検査され、`sorry` は一切含まれていません。
  - 平均回転速度の運動学は**自由度がちょうど1**であり、すべての回転速度が歯数から導かれます。
  - 22個の目標値が成り立ち、メトン周期、サロス周期、エクセリグモス周期、カリポス周期、オリンピアード周期、そして惑星の周期関係も成り立ちます。
  - 37個の中心距離は 10⁻⁶ mm 以内の精度で正確です。
  - ピン・スロット機構は平均回転速度を保ち、月の不等の振幅は 6.579° から 6.581° の間にあります。

  ミューテーションテストによって、誤った歯数、誤った目標値、位置のずれた軸を Lean が棄却することが示されています。
- **機械として実際に動作します。**
...
[elan](https://lean-lang.org/install) を一度インストールしてから、次を実行します。
...
[`lean/README.md`](lean/README.md) には、何が証明されていて何が証明されていないかが、モジュールごとに記載されています。
...
8. **形式的証明。** `tools/make_lean.py` が仕様を Lean 4 に変換します。運動学、目標値、中心距離は、そこから証明されます。ピン・スロット機構の解析と天文学に関する部分は手書きのモジュールで追加し、敵対的レビュアーとミューテーションテストによって、各命題が主張どおりの内容を述べていることを確認しました。
...
- 一部の数量は公表されていません。それらについて仕様では文書化されたデフォルト値を用いており、`spec/antikythera.json` → `unresolved_defaults` に列挙しています。
  ...
  - コスモス表示リングの半径。
  - 初期位相（元期は未較正）。
- Lean による証明が対象としているのは、平均回転速度の運動学、目標値、中心距離、そしてピン・スロット機構とフォロワーの法則です。噛み合い率、歯幅の重なり、アンダーカット、衝突は、Lean ではなく Python ツールと Blender でチェックしています。
