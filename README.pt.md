...
- **Formalmente provado em Lean 4.** [`lean/`](lean/README.md) contém 193 teoremas, verificados pelo núcleo do Lean com o Mathlib, sem nenhum `sorry`:
  - a cinemática das velocidades médias tem exatamente **um grau de liberdade**, e cada velocidade decorre do número de dentes;
  - os 22 valores-alvo verificam-se, tal como os ciclos de Méton, do Saros, do Exeligmos, de Calipo e das Olimpíadas e as relações entre os períodos planetários;
  - as 37 distâncias entre eixos estão corretas com uma precisão de 10⁻⁶ mm;
  - os sistemas de pino e ranhura conservam as suas velocidades médias, e a amplitude da anomalia lunar está entre 6.579° e 6.581°.

  Testes de mutação mostram que o Lean rejeita um número de dentes errado, um valor-alvo errado ou um eixo mal posicionado.
...
### Verificar as provas formais (Lean 4)

Instale o [elan](https://lean-lang.org/install) uma vez e depois:
...
[`lean/README.md`](lean/README.md) lista o que está provado, módulo a módulo, e o que não está.
...
8. **Provas formais.** `tools/make_lean.py` traduz a especificação para Lean 4. A cinemática, os valores-alvo e as distâncias entre eixos são provados a partir dela. Módulos escritos à mão acrescentam a análise dos sistemas de pino e ranhura e a astronomia, e revisores adversariais e testes de mutação verificaram que cada enunciado diz exatamente o que afirma.
...
- As provas em Lean cobrem a cinemática das velocidades médias, os valores-alvo, as distâncias entre eixos e as leis dos sistemas de pino e ranhura e dos seguidores. A razão de contacto, a sobreposição das faces, o subcorte e as colisões são verificados pelas ferramentas Python e no Blender, não no Lean.
