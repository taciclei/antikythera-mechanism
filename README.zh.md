...
- **在 Lean 4 中经过形式化证明**。[`lean/`](lean/README.md) 包含 193 条定理，全部由 Lean 内核借助 Mathlib 检查，没有任何 `sorry`：
  - 平均速率运动学恰好只有**一个自由度**，每个速率都由齿数推导得出；
  - 22 个目标全部成立，默冬、沙罗、三沙罗（Exeligmos）、卡利普斯和奥林匹亚周期以及各行星的周期关系也同样成立；
  - 37 个中心距的误差均在 10⁻⁶ mm 以内；
  - 各销槽机构保持其平均速率，月球近点不等的幅度介于 6.579° 与 6.581° 之间。

  变异测试表明，Lean 会拒绝错误的齿数、错误的目标或位置有误的轴线。
- **在机械上真正可运转**。
...
只需安装一次 [elan](https://lean-lang.org/install)，然后运行：
...
[`lean/README.md`](lean/README.md) 按模块逐一列出了哪些内容已被证明，以及哪些没有。
...
8. **形式化证明**。`tools/make_lean.py` 将规格翻译为 Lean 4，运动学、目标和中心距均据此得到证明。手工编写的模块补充了销槽机构分析和天文学部分，对抗式评审者和变异测试则核查了每条定理陈述确实表达了它所声称的内容。
...
- 部分数值从未发表。对于这些数值，规格采用了有据可查的默认值，列于 `spec/antikythera.json` → `unresolved_defaults`：
  ...
  - 宇宙环的半径；
  - 初始相位（历元未经校准）。
- Lean 证明涵盖平均速率运动学、各项目标、中心距，以及销槽机构和从动件的运动规律。重合度、齿宽重叠、根切和碰撞由 Python 工具和 Blender 进行检查，而不是在 Lean 中证明。
